...
import { Check, Plus, ChevronRight, Menu, X, Eye, FileText, Trash2 } from 'lucide-react';
...
const GetInfo = () => {
  ...
  // console.log('ReceiveData',formData.skills.hardSkills)
  ...
  const handleNext = () => {
    const Fields = {
      ...
      4: formData.projects.length > 0 ? formData.projects.map(proj => [proj.projectTitle, proj.toolsTechUsed, proj.projectDescription]) : [[]],
          ...
          <div className="space-y-4">
            ...
            {(formData.selectedTemplate === '4') && (
              <div className="space-y-2 mt-4 p-4 border-2 border-blue-200 rounded-lg bg-blue-50 dark:bg-slate-700 dark:border-blue-600">
                ...
                />
                {(formData.contactInfo.profileImage) && (
                  <div className="mt-2">
                    <img
                      src={formData.contactInfo.profileImage}
                      alt="Profile Preview"
            ...
            <div className="space-y-4">
              ...
              {formData.projects.map((project, index) => (
                <div key={index} className="p-4 border-2 rounded space-y-4 dark:border-slate-700">
                  ...
                  <div className="space-y-2">
                    <label className="block text-sm font-medium dark:text-slate-300">Project Description</label>
                    <textarea
                      placeholder="A comprehensive task management system built with React and Node.js. Features include real-time collaboration, user authentication, task prioritization, and progress tracking. Implemented RESTful APIs for seamless data flow and responsive UI for cross-device compatibility."
                      className="w-full sm:px-6 sm:p-2 border rounded peer px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-800 dark:text-white dark:border-gray-600 min-h-[100px] resize-y"
                      value={project.projectDescription || ''}
                      onChange={(e) => handleInputChange('projects', 'projectDescription', e.target.value, index)}
                      rows={4}
                    />
                    <div className="ml-4 w-0 h-1 rounded-full bg-blue-500 transition-all duration-300 peer-hover:w-[60%] peer-focus:w-[88%] sm:peer-focus:w-[94%]"></div>
                  </div>
              ...
              {formData.projects.map((project, index) => (
                <div key={index} className="p-4 border-2 rounded space-y-4 dark:border-slate-700">
                  ...
                  <div className="space-y-2">
                    <label className="block text-sm font-medium dark:text-slate-300">Project Description</label>
                    <textarea
                      placeholder="Built an advanced machine translation model from scratch using Transformer architecture. Implemented multi-head attention, positional encoding, and custom training loops. Achieved competitive BLEU scores on English-German translation tasks using Multi30k dataset."
                      className="w-full sm:px-6 sm:p-2 border rounded peer px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-800 dark:text-white dark:border-gray-600 min-h-[100px] resize-y"
                      value={project.projectDescription || ''}
                      onChange={(e) => handleInputChange('projects', 'projectDescription', e.target.value, index)}
                      rows={4}
        ...
        <div className={`${isOpen || "hidden lg:block"}`}>
          ...
          <div className="mt-6 px-2">
            <button
              ...
              className="w-full flex items-center justify-center gap-2 p-3 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-gray-300 transition-all duration-300"
              title="Go back to file upload page"
            >
          ...
          <div>
            <div className="flex items-center mt-4 justify-center gap-2 mr-2 dark:text-slate-300" title='Create your professional resume'>
              <p>Create your</p>
        ...
        <div className={`${isPreviewOpen || "hidden md:block"}`}>
          ...
          <div className="px-2 w-[250px] ml-3 h-[400px] overflow-hidden bg-white dark:bg-slate-800 ">
            <div className="scale-[0.24] origin-top-left flex ">
              {(formData.selectedTemplate == 1) ? <T1 jsonData={formData} /> : (formData.selectedTemplate == 2) ? <T2 jsonData={formData} /> : (formData.selectedTemplate == 3) ? <T3 jsonData={formData} /> : (formData.selectedTemplate == 4) ? <T4 jsonData={formData} /> : (formData.selectedTemplate == 5) ? <T5 jsonData={formData} /> : (formData.selectedTemplate == 6) ? <T6 jsonData={formData} /> : <div></div>}
            </div>
          </div>
          <div className={`whitespace-pre-line dark:text-slate-300 p-3 md:p-1 "block"`}>
