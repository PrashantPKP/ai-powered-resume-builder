...
const FileUploadPage = () => {
    ...
    if (file && file.type === "application/json") {
      try {
        ...
        if (expectedKeys.every((key) => actualKeys.includes(key))){
           ...
           toast.success("JSON file uploaded successfully! Redirecting to editor...", { duration: 2000 , position: "top-right"});
           setTimeout(() => {
             navigate("/GetInfo", { state: { jsonData: dataToCheck } });
           }, 500);
         }else{
    ...
    if (file && (file.type === "text/html" || file.name.endsWith('.html'))) {
      try {
        ...
        if (jsonMatch && jsonMatch[1]) {
          ...
          if (expectedKeys.every((key) => actualKeys.includes(key))) {
            setJsonData(dataToCheck);
            toast.success("HTML file uploaded successfully! Redirecting to editor...", { duration: 2000, position: "top-right" });
            setTimeout(() => {
              navigate("/GetInfo", { state: { jsonData: dataToCheck } });
            }, 500);
          } else {
  ...
  return (
    ...
    {/* Back to Home Button */}
    <div className="w-[90%] lg:w-4/5 mx-4 mt-6">
      <button
        onClick={() => navigate('/')}
        className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-gray-300 transition-all duration-300 shadow-md hover:shadow-lg"
...
{/* 
   ...
   {/* Fill Form */}
    ...
    <div className="w-[90%] lg:w-4/5 mx-4 bg-gray-200 dark:bg-slate-800 rounded-xl shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 py-4 space-y-4 sm:space-y-0 hover:shadow-lg transition-shadow duration-300">
      <div className="w-full sm:w-auto lg:ml-14">
        <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">
          Upload json data..  Structured by us
        </p>
        <p className="text-md mt-1 font-semibold text-gray-500 dark:text-gray-400">
          The fastest way to redesign resume without filling whole details again
        </p>
      </div>
    ...
    {/* Parsed Resume Upload */}
    <div className="w-[90%] lg:w-4/5 mx-4 bg-gray-200 dark:bg-slate-800 rounded-xl shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 py-4 space-y-4 sm:space-y-0 hover:shadow-lg transition-shadow duration-300">
      <div className="w-full sm:w-auto lg:ml-14">
        <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">
          Upload your existing resume to parse data from it
        </p>
        <p className="text-md mt-1 font-semibold text-gray-500 dark:text-gray-400">
    ...
    <div className="w-[90%] lg:w-4/5 mx-4 bg-gray-200 dark:bg-slate-800 rounded-xl shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 py-4 space-y-4 sm:space-y-0 hover:shadow-lg transition-shadow duration-300">
      <div className="w-full sm:w-auto lg:ml-14">
        <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">
          Upload HTML to extract and edit data
        </p>
        <p className="text-md mt-1 font-semibold text-gray-500 dark:text-gray-400">
